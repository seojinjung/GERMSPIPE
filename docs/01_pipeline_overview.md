...
The same way you're inevitably going to have some low-quality read ends, you're going to have duplicate reads. When I started trying to research what duplication is and why we don't like it, it turns out that the answers to both of those questions are actually pretty complicated (which I should have expected lol) and a point of contention in the scientific community. I'll talk about it a bit more in this step's Markdown file.

In this step, we use a tool to identify and tag read pairs that are likely artifactual duplicates from the sequencing process. This makes it so there's only one unmarked copy of each unique read pair. The tagged pairs are usually ignored for the rest of the analysis; for this reason, this step is also called de-duplication. 
