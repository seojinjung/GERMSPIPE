...
To remain true to the spirit of documenting my learning process, please allow me to first go over how I landed on my concept. I started by referencing [this SOP](https://h3abionet.github.io/H3ABionet-SOPs/Variant-Calling) by H3ABioNet to outline the general steps before getting into the details. The most recent reference in their bibliography is from 2018, which likely makes this page about eight years old at the time of writing (2026), but I figured the core framework was still sound. At a later point in my research, I found a more recent paper, "Bioinformatics of germline variant discovery for rare disease diagnostics: current approaches and remaining challenges" (2024) by Barbitoff, *et. al.*, that covers a similar process but goes into more details for each step. Keeping in mind that I wanted to follow GATK's Best Practices guidelines (and also in search of more up-to-date sources), I dug around their website and found these articles on [data pre-processing](https://gatk.broadinstitute.org/hc/en-us/articles/360035535912-Data-pre-processing-for-variant-discovery) and [germline short variant discovery](https://gatk.broadinstitute.org/hc/en-us/articles/360035535932-Germline-short-variant-discovery-SNPs-Indels). I used these three sources as jumping off points for building the blueprint of my pipeline, with additional sources sprinkled in where they became relevant.

## Phase 0. Preparing the reference genome triad
Most modern NGS pipelines require more than just a reference FASTA. You need a FASTA index and a FASTA dictionary as well. The uses for the index and the dictionary can slightly vary by case, but the main point of them is to enable fast random access into the reference genome (which is very large) and cross-check compatibility between the reference, the tools, and downstream files, respectively. Together, these files are considered the ["prepared reference triad"](https://bffo.org/format/Sequence-Dictionary/). As far as I can tell, the main tools that *require* this triad are GATK and Picard, and since I'm likely to use those tools—either from comparing my pipeline to GATK's or using a Picard tool in my own pipeline—it behooves me to follow the standard and generate these files. 

## Phase 1. Read pre-processing
Once we have all our initial files, we need to make sure the data are good enough to be worth analyzing. If the data quality is low, the analysis results won't mean anything. It's kind of an implied step, I guess, but it's good practice to check your raw data before doing anything to it. After the sequences clear the initial screening, they'll need to be prepped for analysis. You could also call this phase quality control, but in my opinion, something akin to "quality control" happens in basically every other step throughout this entire process. Half of the workflow is quality control. You'll see what I mean.
...
![Pipeline overview figure](./img/germspipe_overview.svg "Pipeline overview.")
